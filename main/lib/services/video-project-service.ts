...
export class VideoProjectService extends EventEmitter {
  ...
  constructor() {
    ...
    if (apiKey) {
      ...
      console.log('🎬 VideoProjectService initialized with Gemini');
    } else {
      console.warn('⚠️ GEMINI_API_KEY not found - AI analysis will not work');
    }
  ...
  private async startImageServer(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.imageServer = http.createServer((req, res) => {
        const url = req.url || '/';
        // URL esperada: /images/segment-1-123456.jpg ou /audio-123456.mp3
  ...
  public async analyzeWithAI(
    ...
    this.emit('status', { stage: 'analyzing', message: 'Analisando com IA...' });
    ...
    try {
      ...
      const analyzedSegments = JSON.parse(jsonMatch[0]) as Array<{
        id: number;
        emotion: string;
        imagePrompt: string;
        assetType: string;
      ...
      const updatedSegments = segments.map(seg => {
        const analysis = analyzedSegments.find(a => a.id === seg.id);
        if (analysis) {
          return {
            ...seg,
...
4. **cameraMovement**: Movimento de câmera sugerido:
   - "static" para foco no conteúdo
   - "zoom_in_slow" para criar tensão ou foco
   - "ken_burns" para documentário/narrativo
   - "pan_left" ou "pan_right" para revelar
   - "shake" para urgência
